...
static void CallbackSendMsgArc(__int64 room_id, int error_code, const char *result, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		ChatRoom::SendMsgArcCallback* cb_pointer = (ChatRoom::SendMsgArcCallback* )user_data;
		if (*cb_pointer)
		{
			Json::Value value;
...
static ChatRoom::SendMsgArcCallback* g_cb_send_msg_arc_ = nullptr;
void ChatRoom::RegSendMsgArcCb(const SendMsgArcCallback& cb, const std::string& json_extension/* = ""*/)
{
	...
	return NIM_SDK_GET_FUNC(nim_chatroom_reg_send_msg_arc_cb)(json_extension.c_str(), &CallbackSendMsgArc, g_cb_send_msg_arc_);
}
...
{
	return NIM_SDK_GET_FUNC(nim_chatroom_enter)(room_id, request_enter_data.c_str(), nullptr, json_extension.c_str());
}
...
std::string ChatRoom::CreateTextMessage(const std::string& from_nick
...
{
	Json::Value values;
	values[kNIMChatRoomMsgKeyFromNick] = from_nick;
