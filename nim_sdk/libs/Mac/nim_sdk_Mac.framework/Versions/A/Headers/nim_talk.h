...
/** @fn void nim_talk_reg_message_filter_cb(const char *json_extension, nim_talk_team_notification_filter_func cb, const void *user_data)
  * (全局回调)注册消息是否需要过滤的回调
  * @param[in] json_extension json扩展参数（备用，目前不需要）
  * @param[in] cb		nim_talk_message_filter_func回调函数定义见nim_talk_def.h
  * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
  * @return void 无返回值
  */
